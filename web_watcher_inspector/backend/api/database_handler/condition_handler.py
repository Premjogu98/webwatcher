...
def getTotalDataCount():
    _, data = queryHandler.getQueryAndExecute(
        "SELECT COUNT(data.id) AS count FROM dms_wpw_tenderlinksdata AS data JOIN dms_wpw_tenderlinks AS links ON data.tlid = links.id WHERE links.process_type = 'Web Watcher' AND links.added_WPW = 'Y';",
        fetchone=True,
    )
    return data
    ...
    try:
        if tenderlink or tenderid or wpwflag:
            ...
            _status, data = queryHandler.getQueryAndExecute(
                ...
                WHERE wpwlink.process_type = 'Web Watcher' AND {whereCondition.rstrip("AND").strip()}{";" if tenderid or tenderlink  else  f" LIMIT {limit} OFFSET {offset};"}
            """,
                fetchall=True,
            )
        else:
...
def conditionTWO(xpath, url):
    try:
        ...
        value = (tenderUrlData["id"], url, xpath, "Y", "Text")
        ...
        queryHandler.executeQuery(
            query=f"""UPDATE `dms_wpw_tenderlinks` SET added_WPW = "Y" WHERE tender_link = "{url}";""",
        )
