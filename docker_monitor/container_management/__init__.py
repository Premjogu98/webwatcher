...
class ContainerManagement:
    ...
    def __stopAndRemoveContainers(self):
        ...
        for container in self.DOCKER_CLIENT.containers.list(all=True):
            if "web-watcher-nginx-1" not in container.name and "web-watcher-web-watcher-backend-1" not in container.name and "web-watcher-web-watcher-frontend-1" not in container.name:
    ...
    def monitorContainers(self):
        while True:
            ...
            for idx in range(2):
                ...
                offset += self.__deployContainerWithBatch(offset=offset,container_limit=container_count ,batch_size=batch_size,total_thread=2)
