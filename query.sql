...
FROM
    `tend_dms`.`dms_wpw_tenderlinks` links
    INNER JOIN `tend_dms`.`dms_wpw_tenderlinksdata` data ON links.id = data.tlid
...
FROM
    ...
    INNER JOIN tbl_region re ON links.country = re.Country_Short_Code
WHERE
    links.process_type = 'Web Watcher'
    AND links.added_WPW = 'Y'
    AND data.entrydone = 'Y'
    AND (
        re.Region_Code LIKE '101%'
        OR re.Region_Code LIKE '102%'
        OR re.Region_Code LIKE '104%'
        OR re.Region_Code LIKE '105%'
        OR re.Region_Code LIKE '103304%'
    )
