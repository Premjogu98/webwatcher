...
class DockerManagement:
    ...
    def createContainer(self, container_count, offset, limit, threads, frozen=False):
        ...
        response = self.session.post(
            f"{self.docker_api_url}/containers/create?name={container_name}",
        ...
        # time.sleep(10)
        self.container_running_details[container_id] = {
        ...
        if container_info:
            if not container_info["State"]["Running"]:
                ...
                if self.is_container_frozen(container_id):
                    self.get_docker_container_logs_and_create_new_container(
                        container_id
                    )
        else:
        ...
        while True:
            if container_count != container_limit:
                ...
                self.createContainer(
                    ...
                    offset=offset,
                    limit=batch_size,
                    threads=total_thread,
                )
                offset += batch_size
                console_logger.info(
                    f"OFFSET = {offset} | TOTAL DATA: {self.data_count}"
                )
                container_count += 1
                continue
            else:
        ...
        while True:
            try:
                response = self.session.get(
                    f"{self.docker_api_url}/containers/json?all=1"
                )
                containers = response.json()
                for container in containers:
    ...
    def update_webwatcher_monitor_log(self):
        status, data = self.dbconnection.getQueryAndExecute(
            "SELECT * FROM dms_wpw_bot_run_log ORDER BY entry_date DESC LIMIT 1;",
            fetchall=True,
        )
        if status:
    ...
    def get_docker_container_logs_and_create_new_container(self, container_id):
        ...
        response = self.session.get(
            f"{self.docker_api_url}/containers/{container_id}/logs", params=params
        )
        ...
        if response.status_code == 200:
            ...
            if count:
                ...
                if total == remaining:
                    ...
                    self.remove_from_dict(container_id)
                    console_logger.info(
                        f"Stopping and removing container: {container_id}"
                    )
                    return True
                response = self.session.get(
                    f"{self.docker_api_url}/containers/{container_id}/json"
                )
                DB_DATA_OFFSET = next(
                    (
                        int(i.replace("DB_DATA_OFFSET=", ""))
                        for i in response.json()["Config"]["Env"]
                        if "DB_DATA_OFFSET" in i
                    ),
                    False,
                )
                DB_DATA_LIMIT = next(
                    (
                        int(i.replace("DB_DATA_LIMIT=", ""))
                        for i in response.json()["Config"]["Env"]
                        if "DB_DATA_LIMIT" in i
                    ),
                    False,
                )
                ...
                self.createContainer(
                    offset=MAIN_DB_DATA_OFFSET,
                    limit=MAIN_DB_DATA_LIMIT,
                    threads=2,
                    frozen=True,
                )
                ...
                try:
                    ...
                    console_logger.debug(
                        f"Failed to stop or delete container {container_id}"
                    )
        else:
            console_logger.error(
                f"Failed to retrieve logs. Status code: {response.status_code}"
            )
        return False
    ...
    def convert_to_local_time(self, time_str, time_format):
        ...
        local_time = input_time.replace(tzinfo=datetime.timezone.utc).astimezone(
            local_timezone
        )
        return local_time
    ...
    def get_container_start_end_time(self, container_id):
        ...
        if container_info["State"]["OOMKilled"]:
            if not self.get_docker_container_logs_and_create_new_container(
                container_id
            ):
                return {}
        ...
        console_logger.debug(
            f'{container_info["Name"]} start_time_str : {start_time_str}'
        )
        console_logger.debug(f'{container_info["Name"]} end_time_str : {end_time_str}')
        if start_time_str:
            start_time_str = (
                start_time_str.partition(".")[0].replace("T", " ").replace("Z", "")
            )
            console_logger.debug(
                f'{container_info["Name"]} start_time_str : {start_time_str}'
            )
        ...
        if end_time_str:
            end_time_str = (
                end_time_str.partition(".")[0].replace("T", " ").replace("Z", "")
            )
            console_logger.debug(
                f'{container_info["Name"]} end_time_str : {end_time_str}'
            )
        ...
        return {
            ...
            "difference": str(time_difference),
        }

    def get_container_stats(self, container_id):
        response = self.session.get(
            f"{self.docker_api_url}/containers/{container_id}/stats?stream=false"
        )
        stats = response.json()
        return stats
        ...
        try:
            for _ in range(threshold):
                ...
                if stats["cpu_stats"].get("system_cpu_usage"):
                    ...
                    self.remove_from_dict(container_id)
                    console_logger.info(
                        f"Container Removed {stats['name']} had no system_cpu_usage used stats"
                    )
                    return False
                if stats["memory_stats"].get("usage"):
                    ...
                    self.remove_from_dict(container_id)
                    console_logger.info(
                        f"Container Removed {stats['name']} had no memory used stats"
                    )
                    return False
                ...
                if round(abs(cpu_percent - prev_cpu), 2) > 0.05 or abs(
                    mem_usage - prev_mem
                ) > (
                    60 * 1024 * 1024
                ):  # less than 50 mb
...
if __name__ == "__main__":
    ...
    time.sleep(5)
    ObjDockerManagement.start_process(
        container_limit=55, batch_size=1300, total_thread=2
    )
    # time.sleep(60)
