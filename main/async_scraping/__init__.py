    ...
    async def browseManagement(self, **details: Dict[str, Any]) -> None:
        ...
        try:
            ...
            console_logger.error(
                f"\033[91m EXCEPTION: Error on line {sys.exc_info()[-1].tb_lineno}  EXCEPTION: {error} \033[00m"
            )
        ...
        try:
            ...
            if await order_sent.count() == 0:
                ...
                self.QUERY_HANDLER.error_log(
                    error=f'XPath error {details["XPath"]}', id=details["id"]
                )
