        ...
        if not details["oldHtmlPath"] and not details["newHtmlPath"]:
            ...
            if result:
                if self.fileHandler.generateHtmlFile(htmlstring=details["onlyhtml"],filename=newhtmlfile):
                    self.QUERY_HANDLER.executeQuery(f"""UPDATE dms_wpw_tenderlinksdata SET newHtmlPath = "{newhtmlfile}", compare_per = "{str(compare_per)}", CompareChangedOn = "{self.getCurrentTime()}", entrydone = "N", error_date = "", compare_error = "" WHERE id = {details["id"]}""")
            ...
            if result:
                if self.fileHandler.generateHtmlFile(htmlstring=details["onlyhtml"],filename=newhtmlfile) and self.fileHandler.generateHtmlFile(htmlstring=old_html,filename=oldhtmlfile):
                    self.QUERY_HANDLER.executeQuery(f"""UPDATE dms_wpw_tenderlinksdata SET newHtmlPath = "{newhtmlfile}", oldHtmlPath = "{oldhtmlfile}", compare_per = "{str(compare_per)}", CompareChangedOn = "{self.getCurrentTime()}", LastCompareChangedOn="{details["CompareChangedOn"]}", entrydone = "N" , error_date = "", compare_error = "" WHERE id = {details["id"]}""")
                    self.updateChangesCount(id=details["id"])
                    self.GLOBAL_VARIABLE.compared += 1
                    return True
        else:
