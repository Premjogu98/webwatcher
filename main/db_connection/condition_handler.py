...
class ConditionHandler:
    ...
    def checkConditionBeforeTextComparison(self, **details):
        ...
        if not details["newHtmlPath"]:
            ...
            _, old_text = self.fileHandler.extractInnerText(
                filename=details["newHtmlPath"]
            )
            ObjComparison = Comparison(OLD_TEXT=old_text, NEW_TEXT=details["onlytext"])
            result, compare_per = ObjComparison.startCompare()
            if result:
                if self.fileHandler.renameFile(
                    old_file=details["newHtmlPath"], new_file=oldhtmlfile
                ):
                    if self.fileHandler.generateHtmlFile(
                        htmlstring=details["onlyhtml"], filename=newhtmlfile
                    ):
                        self.QUERY_HANDLER.executeQuery(
                            f"""UPDATE dms_wpw_tenderlinksdata SET oldHtmlPath = "{oldhtmlfile}", newHtmlPath = "{newhtmlfile}", compare_per = "{str(compare_per)}", CompareChangedOn = "{self.getCurrentTime()}", entrydone = "N", error_date = "", compare_error = "" WHERE id = {details["id"]}"""
                        )
            ...
            if result:
                if self.fileHandler.renameFile(
                    ...
                    if self.fileHandler.generateHtmlFile(
                        ...
                        self.QUERY_HANDLER.executeQuery(
                            f"""UPDATE dms_wpw_tenderlinksdata SET newHtmlPath = "{newhtmlfile}", oldHtmlPath = "{oldhtmlfile}", compare_per = "{str(compare_per)}", CompareChangedOn = "{self.getCurrentTime()}", LastCompareChangedOn="{details["CompareChangedOn"]}", entrydone = "N" , error_date = "", compare_error = "" WHERE id = {details["id"]}"""
                        )
                        self.updateChangesCount(id=details["id"])
                        self.GLOBAL_VARIABLE.compared += 1
                        return True
