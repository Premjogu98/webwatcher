...
class QueryHandler:
    ...
    def error_log(self,error,id):
        self.getQueryAndExecute(f"""UPDATE dms_wpw_tenderlinksdata_test SET compare_error ="{error.replace("'","")}", error_date="{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}" WHERE id = {id}""")
